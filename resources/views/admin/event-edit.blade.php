                    ...
                    <form action="{{ route('admin.events.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="col">
                                ...
                                <input type="text" class="form-control" name="subtitle" value="{{ $event->subtitle }}">
                                @error('subtitle')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" onclick="$('#blacklist-textarea').toggle()">
                                        ...
                                        @if ($event->dates()->count() > 0)
                                            <table class="table">
                                                ...
                                                <tr>
                                                    ...
                                                    <th scope="col">{{ __('app.date.enrollment_start') }}</th>
                                                    <th scope="col">{{ __('app.date.enrollment_end') }}</th>
                                                    <th scope="col">{{ __('app.date.withdraw_end') }}</th>
                                                ...
                                                <tbody>
                                                    @foreach($event->dates as $date)
                                                        <tr>
                                                            <td>{{ \Carbon\Carbon::parse($date->date_start)->format('d.m.Y H:i') }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($date->date_end)->format('d.m.Y H:i') }}</td>
                                                            <td>{{ $date->capacity ?? '-' }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($date->date_end)->format('d.m.Y H:i') }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($date->date_end)->format('d.m.Y H:i') }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($date->date_end)->format('d.m.Y H:i') }}</td>
                                                            <td>{{ $date->name ?? '-' }}</td>
                                                            <td><button type="submit" title="Odstranit" class="btn btn-outline-danger btn-rounded"> <i class="fas fa-trash"></i></button></td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        @if (isset($event->c_fields))
                                            <table class="table">
                                                <thead>
                                                <tr>
                                                    <th scope="col">{{ __('app.c_fields.name') }}</th>
                                                    <th scope="col">{{ __('app.c_fields.type') }}</th>
                                                    <th scope="col">{{ __('app.c_fields.required') }}</th>
                                                    <th scope="col">{{ __('app.c_fields.default') }}</th>
                                                    <th></th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                @foreach(json_decode($event->c_fields) as $name => $tag)
                                                    <tr>
                                                        <th scope="col">|{{ $name }}|</th>
                                                        <th scope="col">{{ $tag->type }}</th>
                                                        <th scope="col">{{ $tag->required ? 'Ano' : 'Ne' }}</th>
                                                        <th scope="col">{{ $tag->default ?? '-' }}</th>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                            </table>
                                        @else
                                            <p>Nejsou vytvořeny žádné vlastní pole.</p>
                                        @endif
                                    </div>
                                </div>
